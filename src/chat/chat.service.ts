  ...
  async sendMessage(
    ...
    try {
      ...
      if (response.images?.length) {
        imageResponses = response.images.map(
          (img) => new ChatImageResponse(img.imageId, img.imageUrl),
        );
      }

      LoggingUtil.log(
        'ChatService',
        `메시지 전송 성공: roomId=${message.roomId}, memberId=${memberId}`,
      );
      ...
      LoggingUtil.error(
        'ChatService',
        `메시지 전송 실패: roomId=${message.roomId}, memberId=${memberId}`,
        error,
      );
      throw error;
    }
